...
from src.models.fusion import AutoencoderFusion, DegreeConditionedFusion
...
def _phase1_analytical_calibration(
    ...
    try:
        ...
        if n_violations >= 2:
            raise RuntimeError(
                "Gate calibration failed: alpha is non-monotonic across "
                f"quartiles. Pairs: {diag_pairs}, "
                f"fitted (w, b) = ({w_fit:.4f}, {b_fit:.4f}). "
                "This indicates fundamental design failure — LLM-vs-graph "
                "trade-off is not coherent with degree."
            )
        if n_violations == 1 or w_fit <= 0:
            logger.warning(
                "Gate calibration produced borderline result: "
                f"pairs={diag_pairs}, (w, b)=({w_fit:.4f}, {b_fit:.4f}). "
...
def _supervised_finetune(
    ...
    # Trainable params: PhenoDrugModel params (filtered for requires_grad)
    # plus any AE encoder params passed in via extra_trainable_params.
    ...
    for epoch in range(finetune_epochs):
        ...
        for b_start in range(0, len(pairs), batch_size):
            ...
            if not masking_check_done and use_anchoring and epoch == 0 and n_batches == 1:
                ...
                with torch.no_grad():
                    with torch.autocast(
                        ...
                        h_fused_dbg = masked_fusion(h_graph_dbg, h_llm_full, *extras_dbg)
    ...
    logger.info(
        ...
        f"(best_val_mrr={best_val_mrr:.4f}, use_amp={use_amp})"
    )
    return {"best_val_mrr": best_val_mrr}
...
def run_degree_cond_experiment(config: dict) -> dict:
    ...
    `late_fusion_eval.run_late_fusion_experiment`):
        ...
        margin, neg_ratio, accum_steps, grad_clip, batch_size, seed,
        device, results_dir.
    """
    import wandb
    t0 = perf_counter()
    ...
    # log_degree from KG structural edge_index (NOT _build_drug_degree)
    log_degree = compute_log_degree(edge_index, num_nodes).to(device)
    logger.info(
        f"run_degree_cond_experiment: computed log_degree in {_fmt_elapsed(t0)}"
    )
    ...
    if bool(config.get("skip_phase1", False)):
        ...
        diag = _phase1_analytical_calibration(
            ...
            h_llm_full=h_llm_full, log_degree=log_degree,
    ...
    return {
        ...
        "log_degree": log_degree.detach().cpu(),
        "drug_indices_arr": drug_indices_arr,
        "per_disease_df": df,
...
def _phase1_ae_pretraining(
    ...
    logger.info(f"AE pretraining: n_rows_in_loss = {n_text}")
    if wandb_run is not None:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            logger.info(f"AE pretrain epoch {epoch+1}/{epochs} loss={losses[-1]:.6f}")
    return losses


def run_autoencoder_experiment(config: dict) -> dict:
    """Top-level entry: autoencoder fusion (Approach b)."""
    import wandb
    ...
    run_name = f"feature-fusion-ae-{encoder_name}-{desc_tier}-{projection}"
    wandb.init(project="rare-disease-repurposing", name=run_name)
    wandb.config.update(config)
    wandb.define_metric("epoch")
    wandb.define_metric("train/*", step_metric="epoch")
    ...
    assert fusion.encoder[0].in_features == 2 * embed_dim, (
        f"AE first Linear in_features={fusion.encoder[0].in_features} != 2*{embed_dim}"
    )
    masked_fusion = MaskedFusionWrapper(fusion, has_text_mask).to(device)
    ...
    for p in fusion.decoder.parameters():
        p.requires_grad_(False)
    ...
    results_dir = Path(config.get("results_dir", "results/tables"))
    csv_path = results_dir / (
        f"feature_fusion_autoencoder_{encoder_name}_{desc_tier}_{projection}.csv"
    )
    df = _per_disease_csv(
        scores_ind=scores_test, scores_off=scores_test,
        truth_ind={d: set(v) for d, v in test_disease_to_drugs.items()},
